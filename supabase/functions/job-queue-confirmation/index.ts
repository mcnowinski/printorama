...
serve(async (req) => {
  ...
  try {
    ...
    const emailBody = [
      ...
      '',
      `Your job request "${record.title || 'Untitled'}" has been received and is now in the review queue.`,
      '',
      'A lab manager will review it shortly. You will receive an email when the status changes.',
      '',
      statusLink ? `Check your status: ${statusLink}` : '',
      '',
      'Thanks,',
      senderName,
    ].filter(Boolean).join('\n')
